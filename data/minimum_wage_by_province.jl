...
<head>
  ...
  <title>Provincial Minimum Wages - Canada &mdash; Inequality Dashboard</title>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    #table-wrap { display: none; overflow: auto; max-height: 420px; }
    #table-wrap.open { display: block; }

    #history-table { width: 100%; border-collapse: collapse; font-size: 0.8rem; }
    #history-table th, #history-table td { text-align: left; padding: 0.4rem 1rem; border-bottom: 1px solid var(--border); }
    #history-table th { position: sticky; top: 0; background: var(--card-bg); font-weight: 600; color: var(--muted); font-size: 0.72rem; text-transform: uppercase; letter-spacing: 0.05em; }
    #history-table td.note { color: var(--muted); font-size: 0.75rem; }
...
<div class="page" style="padding-bottom: 1rem;">
  ...
  <h2 class="post-title">Provincial Minimum Wages - Canada</h2>
</div>
...
<main class="page" style="padding: 1.5rem 1.5rem;">
  ...
  <section id="rankings">
    <div class="section-label">Current General Minimum Wage</div>
    <div class="rank-controls">
      <span class="rank-ctrl-label">As of September 2, 2026 &mdash; click a province to highlight it</span>
    </div>
    <div id="rank-list" class="rank-list"></div>
  </section>

  <section class="about-section">
    <button id="table-toggle" onclick="toggleTable()">&#9662; Show full data table for selected province</button>
    <div id="table-wrap">
      <table id="history-table">
        <thead><tr><th>Effective date</th><th>Rate (\$/hr)</th><th>Note</th></tr></thead>
    ...
    <div class="prose">
      ...
      <p>
        The <strong>general (basic adult) minimum hourly wage</strong> in each of Canada's ten provinces, plus their unweighted average, from January 1, 1974 &mdash; the date every province has a comparable general rate &mdash; through the present. The average line is shown by default; click any province, or a row in the ranking list, to highlight it instead.
      </p>

      <h2>Data source</h2>
      <p>
        Employment and Social Development Canada (ESDC), <a href="https://open.canada.ca/data/en/dataset/390ee890-59bb-4f34-a37c-9732781ef8a0" target="_blank" rel="noopener">"Historical Minimum Wage Rates in Canada"</a> (dataset 390ee890-59bb-4f34-a37c-9732781ef8a0, resource <code>wages_general-2.csv</code>), Government of Canada Open Government Portal, downloaded September 2, 2026. This is the same underlying data behind the government's public <a href="https://minwage-salairemin.service.canada.ca/" target="_blank" rel="noopener">Minimum Wage Database</a>.
      </p>
      <p>
        British Columbia's June 2025 and June 2026 rates were corrected against BC government news releases (<a href="https://news.gov.bc.ca/releases/2025LBR0017-000500" target="_blank" rel="noopener">2025LBR0017-000500</a>, <a href="https://news.gov.bc.ca/releases/2026LBR0021-000581" target="_blank" rel="noopener">2026LBR0021-000581</a>): the source file mislabeled the \$17.85 rate as effective June 1, 2026 instead of June 1, 2025.
      </p>
      ...
      <ul>
        ...
        <li>Putting both exclusions together, each province's sourced record begins at the first date a single rate, with no sex, regional, or sectoral differentiation, covered the whole province: Ontario &mdash; December 27, 1965; British Columbia &mdash; January 1, 1965 (never split); Manitoba &mdash; December 1, 1966; Alberta &mdash; July 1, 1966; New Brunswick &mdash; January 1, 1968; Newfoundland and Labrador &mdash; June 1, 1972; Nova Scotia &mdash; July 1, 1972; Quebec &mdash; May 1, 1971; Saskatchewan &mdash; January 2, 1972; Prince Edward Island &mdash; January 1, 1974.</li>
        <li><strong>The chart and data table display every province from January 1, 1974 onward</strong> &mdash; Prince Edward Island's start date, the latest of the ten &mdash; even though most provinces' sourced record (above) goes back further, so every province is shown over the same span and is directly comparable. For a province whose record starts earlier, the displayed series' first point holds whatever rate was already in effect on January 1, 1974 (noted on hover and in the table); no earlier data is discarded, only left out of this display.</li>
        ...
        <li>Each province's line is extended flat to today at its last recorded rate (e.g. Alberta's line continues at \$15.00 through the present) so a rate that simply hasn't changed recently still reads as in effect, rather than the line appearing to end; this doesn't add a new rate change, only a visual continuation, and is noted on hover.</li>
        <li><strong>Canada (average)</strong> is the unweighted mean of the ten provinces' rates, shown as a dashed line, red when selected (provinces turn blue when selected). It runs the same January 1, 1974 through today span as the provinces, so it's always computed across all ten.</li>
      ...
      <p class="source">
        Chart created by the Stone Centre on Wealth and Income Inequality at the Vancouver School of Economics (UBC).
      </p>
    </div>
  </section>
</main>
...
<script>
  ...
  const AVERAGE_NAME = "Canada (average)";
  ...
  // Island's, 1974-01-01). The chart and the data table both start here for
  // every province, so all ten (and the average) are shown over the same,
  ...
  function latestRate(name) {
    var d = name === AVERAGE_NAME ? AVERAGE_SERIES : DATA[name];
    return d.rates[d.rates.length - 1];
  }

  function renderRanking() {
    var entries = PROVINCES.concat([AVERAGE_NAME]).map(function(name) {
      return { name: name, value: latestRate(name) };
  ...
  function renderTable(name) {
    var d = name === AVERAGE_NAME ? AVERAGE_SERIES : displaySeries(name);
    document.getElementById("history-body").innerHTML = d.dates.map(function(date, i) {
      return '<tr><td>' + date + '</td><td>\$' + d.rates[i].toFixed(2) + '</td><td class="note">' + (d.notes[i] || '') + '</td></tr>';
    }).join('');
  }
  ...
  function toggleTable() {
    ...
    btn.innerHTML = (wrap.classList.contains("open") ? "&#9652; Hide" : "&#9662; Show") + " full data table for selected province";
  }

  Plotly.newPlot("chart", buildTraces(currentProvince), buildLayout(), { responsive: true });
  renderRanking();
  renderTable(currentProvince);
