...
<head>
  ...
  <title>Minimum Wage by Province &mdash; Inequality Dashboard</title>
...
<div class="page" style="padding-bottom: 1rem;">
  ...
  <h2 class="post-title">Minimum Wage by Province</h2>
</div>
...
<script>
  ...
  function buildLayout() {
    return {
      title: { text: "General Minimum Wage by Province, 1965–Present" },
  ...
  function latestRate(prov) {
    var d = DATA[prov];
    return d.rates[d.rates.length - 1];
  }

  function renderRanking() {
    var entries = PROVINCES.map(function(prov) {
      return { province: prov, value: latestRate(prov) };
    }).sort(function(a, b) { return b.value - a.value; });

    document.getElementById("rank-list").innerHTML = entries.map(function(e, i) {
      var cls = "rank-row" + (e.province === currentProvince ? " rank-active" : "");
      return '<div class="' + cls + '" onclick="selectProvince(\\'' + e.province + '\\')">'
           + '<span class="rank-num">' + (i + 1) + '</span>'
           + '<span class="rank-name">' + e.province + '</span>'
           + '<span class="rank-val">\$' + e.value.toFixed(2) + '</span>'
           + '</div>';
    }).join('');
  }

  function renderTable(prov) {
    var d = DATA[prov];
