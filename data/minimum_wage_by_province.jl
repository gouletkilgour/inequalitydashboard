...
<main class="page" style="padding: 1.5rem 1.5rem;">
  ...
  <section class="about-section">
    ...
    <div class="prose">
      ...
      <p>
        The <strong>general (basic adult) minimum hourly wage</strong> in each of Canada's ten provinces, from the earliest year each province set a single, sex-neutral rate through the present.
      </p>
      ...
      <ul>
        ...
        <li><strong>Canada (average)</strong> is the unweighted mean of the ten provinces' rates, shown as a dashed line. It starts January 1, 1974 &mdash; the latest of the ten provinces' start dates (Prince Edward Island's) &mdash; so the average is always computed across all ten provinces rather than jumping as provinces with earlier data drop in.</li>
...
<script>
  ...
  // A province's series extended with one extra point at TODAY (holding the
  // last known rate) if its last recorded change predates today, so the line
  // visibly continues through the present instead of appearing to end.
  function extended(prov) {
    var d = DATA[prov];
    var dates = d.dates.slice();
    var rates = d.rates.slice();
  ...
  // province's rate changes. Starts once every province has a general rate
  // in effect (the latest of the ten start dates, Prince Edward Island's
  // 1974-01-01) so the average isn't distorted by provinces dropping in and
  // out of a shifting, smaller set as their series begin at different dates.
  function computeAverageSeries() {
    var start = PROVINCES.map(function(p) { return DATA[p].dates[0]; }).sort().pop();
    var dateSet = {};
    PROVINCES.forEach(function(p) {
      DATA[p].dates.forEach(function(dt) { if (dt >= start) { dateSet[dt] = true; } });
    });
    dateSet[start] = true;
    dateSet[TODAY] = true;
  ...
  function buildTraces(selected) {
    ...
    traces.push({
      ...
      line: { shape: "hv", color: avgSel ? HIGHLIGHT_COLOR : AVERAGE_COLOR, width: avgSel ? 3 : 2, dash: avgSel ? "solid" : "dot" },
      opacity: 1,
